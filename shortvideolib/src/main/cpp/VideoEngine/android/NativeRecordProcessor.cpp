#include "com_mlingdu_demo_recorder_NativeRecordProcessor.h"
...
JNIEXPORT jint JNICALL Java_com_mlingdu_demo_recorder_NativeRecordProcessor_init(JNIEnv * env, jobject obj,
...
JNIEXPORT jint JNICALL Java_com_mlingdu_demo_recorder_NativeRecordProcessor_pushAudioBufferToQueue(JNIEnv * env, jobject obj, jint index, jshortArray audioSamples,
...
JNIEXPORT void JNICALL Java_com_mlingdu_demo_recorder_NativeRecordProcessor_flushAudioBufferToQueue(JNIEnv * env, jobject obj, jint index) {
	if(handles[index] != 0 ) {
		RecordProcessor* recorder = handles[index];
...
JNIEXPORT void JNICALL Java_com_mlingdu_demo_recorder_NativeRecordProcessor_destroy(JNIEnv * env, jobject obj, jint index) {
	if(handles[index] != 0 ) {
		RecordProcessor* recorder = handles[index];
		recorder->destroy();
		delete recorder;
