...
void VideoConsumerThread::stop() {
	...
	if (NULL != statistics && NULL != onStatisticsCallback) {
		...
		onStatisticsCallback(statistics->getStartTimeMills(), statistics->getConnectTimeMills(),
							 duration, statistics->getDiscardFrameRatio(),
...
int64_t VideoConsumerThread::getVideoDuration() {
	return videoPublisher->getVideoDuration();
}
