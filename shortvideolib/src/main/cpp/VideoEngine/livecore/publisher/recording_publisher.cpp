...
int RecordingPublisher::init(char *videoOutputURI,
    ...
    memcpy(outputURI, videoOutputURI, sizeof(char) * strlen(videoOutputURI));

    /* 2:allocate the output media context */
    avformat_alloc_output_context2(&oc, NULL, "flv", videoOutputURI);
    if (!oc) {
        LOGI("avformat_alloc_output_context2 failed");
        return -1;
    }
    fmt = oc->oformat;
...
int64_t RecordingPublisher::getVideoDuration() {
    return duration;
}
